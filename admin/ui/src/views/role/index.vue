    ...
    <div class="filter-container">
      <el-row :gutter="10">
        <el-col :xs="6" :sm="4" :md="4" :lg="4">
          <el-input v-model="searchData.name" placeholder="角色名称" clearable></el-input>
        </el-col>
...
export default {
  ...
  methods: {
    ...
    handleUpdate(innerid) {
      this.$router.push('/role/roleEdit/' + innerid)
    },
    // 新增
    handleAdd() {
      this.$router.push('/role/roleAdd')
    },
    //删除
    handleDelete(innerid) {
      this.$confirm('确定删除该条数据吗?', '提示', {
        ...
        del(innerid).then(res => {
          this.$message({message: '操作成功', type: 'success', offset: '60'});
          this.listLoading = false
          this.getList();
        }).catch(err => {
          this.$message({message: err.message || '操作失败', type: 'warning', offset: '60'});
          this.listLoading = false
        })
      }).catch(() => {
      });
